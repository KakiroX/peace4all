...
const setupLesson = (data) => {
	...
	// Module lock guard: check if this chapter is accessible
	if (data.membership && !isAdmin.value) {
		checkModuleLock(parseInt(props.chapterNumber))
	}
...
const checkModuleLock = async (chapterNum) => {
	try {
		const outlineData = await call('lms.lms.utils.get_course_outline', {
			course: props.courseName,
			progress: true,
		})
		if (!outlineData || chapterNum <= 1) return

		// Find the chapter index (chapters are 1-indexed via idx)
		const chapterIndex = outlineData.findIndex(ch => ch.idx === chapterNum)
		if (chapterIndex <= 0) return

		const prevChapter = outlineData[chapterIndex - 1]
		if (!prevChapter?.lessons?.length) return

		const allPrevComplete = prevChapter.lessons.every(l => l.is_complete)
		if (!allPrevComplete) {
			toast.warning(__('Complete the previous module before accessing this one.'))
			router.push({
				name: 'CourseDetail',
				params: { courseName: props.courseName },
			})
		...
		console.error('Module lock check failed:', err)
	}
}
