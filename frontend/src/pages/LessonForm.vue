...
const generateQuizResource = createResource({
	url: 'lms.lms.api.quiz_generator.generate_quiz_for_lesson',
	makeParams(values) {
		return {
	...
	onSuccess(data) {
		...
		toast({
			title: __('Success'),
			text: data.message || __('Quiz generated successfully!'),
			icon: 'check',
			iconClasses: 'text-green-600',
		})
		if (data.quiz_name) {
			// Reload the page or just add the quiz to the UI
			// Ideally we would fetch the lesson details again
	...
	onError(error) {
		...
		toast({
			title: __('Error'),
			text: error.message || __('Failed to generate quiz'),
			icon: 'x',
...
const generateQuiz = () => {
	if (!lessonDetails.data?.lesson.name) {
		toast({
			title: __('Error'),
			text: __('Please save the lesson first before generating a quiz.'),
			icon: 'x',
			iconClasses: 'text-red-600',
		})
		return
