					...
					<div class="chapter-item">
						...
						>
							<DisclosureButton
								...
								:class="{ 'opacity-50 cursor-not-allowed': isChapterLocked(index) }"
							>
								<Lock
									v-if="isChapterLocked(index)"
									class="h-4 w-4 text-ink-gray-5 stroke-1.5 flex-shrink-0"
								/>
								<ChevronRight
									v-else
									:class="{
								...
								<div
									...
									:class="isChapterLocked(index) ? 'text-ink-gray-5' : 'text-ink-gray-9'"
									@click="redirectToChapter(chapter)"
								>
									{{ chapter.title }}
									<span v-if="isChapterLocked(index)" class="text-xs text-ink-gray-4 ml-2">
										{{ __('Complete previous module to unlock') }}
									</span>
								</div>
							...
							<DisclosurePanel v-if="!chapter.is_scorm_package && !isChapterLocked(index)">
								<Draggable
...
const isChapterLocked = (chapterIndex) => {
	// Admins and editors can always access all chapters
	if (props.allowEdit) return false
	// First chapter is never locked
	if (chapterIndex === 0) return false
	// If progress tracking is not enabled, don't lock
	if (!props.getProgress) return false
	if (!outline.data) return false

	// Check if ALL lessons in the previous chapter are complete
	const prevChapter = outline.data[chapterIndex - 1]
	if (!prevChapter || !prevChapter.lessons || prevChapter.lessons.length === 0) return false

	const allPrevComplete = prevChapter.lessons.every(lesson => lesson.is_complete)
